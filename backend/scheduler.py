...
Executa a auditoria/diagnóstico de sistema automaticamente todos os dias às 08:00h.
"""
import threading
import time
...
def start_audit_scheduler(app):
    """Inicia a thread em segundo plano responsável pelo agendamento diário das 08:00h."""
    ...
    def scheduler_loop():
        ...
        print("⏰ [Scheduler] Agendador de Auditoria Automática (08:00h) inicializado.")
        while True:
            try:
                ...
                # Executa todo dia na janela das 08:00 AM (08:00:00 a 08:00:59)
                if now.hour == 8 and now.minute == 0:
                    if _last_audit_date != today_str:
                        with app.app_context():
                            ...
                            try:
                                log_entry = Log(
                                    usuario="SISTEMA (Automação 08:00h)",
                                    acao=f"Auditoria Diária Automática das 08:00h concluída: {result}"
                                )
